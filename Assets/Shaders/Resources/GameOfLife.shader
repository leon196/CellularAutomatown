Shader "Hidden/GameOfLife"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				int count = 0;
				count += step(0.5, tex2D(bitmap, p + float2(-1.0, -1.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2( 0.0, -1.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2( 1.0, -1.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2(-1.0,  0.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2( 1.0,  0.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2(-1.0,  1.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2( 0.0,  1.0) / resolution).r);
				count += step(0.5, tex2D(bitmap, p + float2( 1.0,  1.0) / resolution).r);
